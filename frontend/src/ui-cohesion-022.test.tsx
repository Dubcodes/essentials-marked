...
describe('0.2.2 Admin pairing and dashboard',()=>{
  it('starts with QR, reveals words once, toggles locally, and opens the shared roll from the header',async()=>{
    ...
    const host=await render(<AdminConsole/>);const dashboard=host.querySelector('.dashboard-grid')!;expect([...dashboard.children].map(child=>child.className)).toEqual(['dashboard-section dashboard-overview','dashboard-section dashboard-attention','dashboard-section dashboard-status','dashboard-activity']);const evidence=[...dashboard.querySelectorAll('.metric-card')].find(card=>card.textContent?.includes('attendance evidence issues'))!;expect(evidence.tagName).not.toBe('BUTTON');await act(async()=>{([...host.querySelectorAll('button')].find(button=>button.textContent==='Devices')as HTMLButtonElement).click()});
