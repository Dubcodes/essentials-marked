...
export function Presence(props:WorkflowProps){
 ...
 return <WorkflowWorkspace title="Attendance / Presence" close={props.close}><div className="picker-tools"><ClearableSearch inputRef={searchRef} label="Search children" value={q} onChange={setQ}/><button type="button" className={presentOnly?'active':'minor'} onClick={()=>setPresentOnly(value=>!value)}>Present</button></div>{groups.map(([heading,children])=>children.length?<section className="roster-group" key={heading}>{(()=>{const info=rosterGroupInfo(heading,props.data.rooms,props.roomId);return <h3 className={info.className} style={{'--room-accent':info.accent} as any}><span>{info.icon}</span>{heading}</h3>})()}<div className="child-roster">{children.map(child=>{const action=actionFor(child);return <div className="presence-row" key={`${heading}-${child.id}`}><ChildRosterIdentity child={child} state={physicalRoomStatus(child,props.roomId,names)}/>{action?<button className={action[0]==='depart'?'danger':''} onClick={()=>void act(child,action[0])}>{action[1]}</button>:<small>Absent</small>}</div>})}</div></section>:null)}</WorkflowWorkspace>;
}
