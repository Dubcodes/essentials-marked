...
from sqlalchemy import String, Date, DateTime, Boolean, ForeignKey, Integer, Text, UniqueConstraint, JSON, Index, text
...
class Account(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=uid)
    centre_id: Mapped[str] = mapped_column(ForeignKey('centres.id'), index=True)
    login_id: Mapped[str] = mapped_column(String(255), unique=True, index=True); email: Mapped[str | None] = mapped_column(String(255), unique=True, nullable=True); password_hash: Mapped[str] = mapped_column(String(255)); role: Mapped[str] = mapped_column(String(30), default='admin'); active: Mapped[bool] = mapped_column(Boolean, default=True)
class Child(Base):
    ...
    first_name: Mapped[str] = mapped_column(String(100)); middle_name: Mapped[str | None] = mapped_column(String(100)); last_name: Mapped[str] = mapped_column(String(100), default=''); preferred_name: Mapped[str | None] = mapped_column(String(100)); dob: Mapped[str | None] = mapped_column(String(10)); active: Mapped[bool] = mapped_column(Boolean, default=True)
class Parent(Base):
...
class Device(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=uid); centre_id: Mapped[str] = mapped_column(ForeignKey('centres.id'), index=True); label: Mapped[str] = mapped_column(String(120)); mode: Mapped[str] = mapped_column(String(20), default='classroom'); default_room_id: Mapped[str | None] = mapped_column(ForeignKey('rooms.id')); created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=now); last_active_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=now); revoked: Mapped[bool] = mapped_column(Boolean, default=False)
class Pairing(Base):
    __tablename__ = 'pairings'
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=uid); centre_id: Mapped[str] = mapped_column(ForeignKey('centres.id')); room_id: Mapped[str | None] = mapped_column(ForeignKey('rooms.id')); label: Mapped[str]=mapped_column(String(120),default='Classroom tablet'); mode: Mapped[str] = mapped_column(String(20), default='classroom'); token_hash: Mapped[str] = mapped_column(String(255), unique=True); challenge: Mapped[str] = mapped_column(String(6)); expires_at: Mapped[datetime] = mapped_column(DateTime(timezone=True)); consumed_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True)); device_id: Mapped[str|None]=mapped_column(ForeignKey('devices.id'))
class Audit(Base):
...
class ChildAlert(Base):
    __tablename__='child_alerts'; __table_args__=(Index('ix_open_child_alert','centre_id','child_id','type','resolved_at'),)
    id: Mapped[str]=mapped_column(String(36),primary_key=True,default=uid); centre_id: Mapped[str]=mapped_column(ForeignKey('centres.id'),index=True); child_id: Mapped[str]=mapped_column(ForeignKey('children.id'),index=True); type: Mapped[str]=mapped_column(String(50)); event_id: Mapped[str|None]=mapped_column(ForeignKey('events.id')); created_at: Mapped[datetime]=mapped_column(DateTime(timezone=True),default=now); created_by_staff_id: Mapped[str|None]=mapped_column(ForeignKey('staff.id')); touched_at: Mapped[datetime]=mapped_column(DateTime(timezone=True),default=now); resolved_at: Mapped[datetime|None]=mapped_column(DateTime(timezone=True)); resolved_by_staff_id: Mapped[str|None]=mapped_column(ForeignKey('staff.id'))
