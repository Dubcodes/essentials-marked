import os, secrets, hashlib, json, re, io, base64, asyncio, threading
...
# This compact in-process sequence is suitable for the current single-process
# deployment.  A reconnect, foreground/online reconciliation and slow client
# fallback mean a process restart or missed signal cannot leave a view stale.
LIVE_SEQUENCE:dict[str,int]={}
LIVE_LOCK=threading.Lock()
def publish_change(centre_id:str):
    with LIVE_LOCK:
        LIVE_SEQUENCE[centre_id]=LIVE_SEQUENCE.get(centre_id,0)+1
def live_sequence(centre_id:str):
    with LIVE_LOCK:
        return LIVE_SEQUENCE.get(centre_id,0)
def request_centre_id(request:Request):
    """Resolve a successful writer's centre after its transaction committed."""
    from .db import SessionLocal
    for kind in ('admin','parent','device'):
...
@app.middleware('http')
async def signal_committed_change(request:Request,call_next):
    response=await call_next(request)
    # The response is only emitted after each ordinary API handler commits.
    # Authentication-only operations do not need a data reconciliation signal.
    if request.method in {'POST','PUT','PATCH','DELETE'} and response.status_code<400 and request.url.path.startswith('/api/') and not request.url.path.startswith('/api/auth/'):
        centre_id=request_centre_id(request)
        if centre_id: publish_change(centre_id)
    return response
...
def live_centre(request:Request,db:Session):
    ...
    if request.cookies.get('admin'):
        return admin(request,db).centre_id
    if request.cookies.get('device'):
        return device(request,db).centre_id
    if request.cookies.get('parent'):
        return parent(request,db).centre_id
    raise HTTPException(401,'Sign in required')

@app.get('/api/live')
async def live_events(request:Request,db:Session=Depends(get_db)):
    centre_id=live_centre(request,db)
    async def stream():
        sequence=live_sequence(centre_id)
        # A connection/reconnection itself is an explicit reconciliation cue.
        yield f"event: reconcile\ndata: {json.dumps({'sequence':sequence,'reconnect':True})}\n\n"
        while True:
            if await request.is_disconnected():
                break
            current=live_sequence(centre_id)
            if current!=sequence:
                sequence=current
...
def clean_domain_data(value):
    ...
    return value
PARENT_TEXT_REPLACEMENTS={'shit':'poo','piss':'pee','fuck':'fudge'}
PARENT_TEXT_PATTERN=re.compile(r'(?<!\w)('+'|'.join(map(re.escape,PARENT_TEXT_REPLACEMENTS))+r')(?!\w)',re.IGNORECASE)
def soften_parent_text(value:str|None):
...
def centre_local_datetime(value:str,timezone_name:str,fold:int|None=None):
    try:
        ...
        raise HTTPException(422,'Enter a valid pickup date and time')
    if local.tzinfo is not None:raise HTTPException(422,'Enter pickup time in the Centre timezone')
    zone=ZoneInfo(timezone_name);candidates=[]
    for occurrence in (0,1):
        aware=local.replace(tzinfo=zone,fold=occurrence);instant=aware.astimezone(timezone.utc)
        if instant.astimezone(zone).replace(tzinfo=None)==local and all(existing[1]!=instant for existing in candidates):candidates.append((occurrence,instant))
    if not candidates:raise HTTPException(422,'That local time does not exist because of a daylight-saving clock change')
    if len(candidates)>1 and fold is None:raise HTTPException(422,'That local time occurs twice because of a daylight-saving clock change; choose first or second occurrence')
    if fold is not None:
        chosen=next((instant for occurrence,instant in candidates if occurrence==fold),None)
        if chosen:return chosen
        raise HTTPException(422,'Choose a valid daylight-saving occurrence')
    return candidates[0][1]
ALERT_LABELS={'nappies_low':'Nappies running low','nappies_out':'Nappies out','spare_clothes_needed':'Spare clothes needed','soiled_clothes':'Soiled clothes to take home'}
def alert_out(row:ChildAlert,db:Session):
    child=db.get(Child,row.child_id)
    return {'id':row.id,'child_id':row.child_id,'child_name':' '.join(part for part in [child.preferred_name or child.first_name if child else '',child.last_name if child else ''] if part),'type':row.type,'label':ALERT_LABELS.get(row.type,row.type),'created_at':row.created_at,'touched_at':row.touched_at,'resolved_at':row.resolved_at}
def touch_alert(db:Session,centre_id:str,child_id:str,alert_type:str,event:Event,staff_id:str):
    row=db.scalar(select(ChildAlert).where(ChildAlert.centre_id==centre_id,ChildAlert.child_id==child_id,ChildAlert.type==alert_type,ChildAlert.resolved_at.is_(None)).order_by(ChildAlert.created_at.desc()))
    if row:
        row.touched_at=now();row.event_id=event.id
        return row
    row=ChildAlert(centre_id=centre_id,child_id=child_id,type=alert_type,event_id=event.id,created_by_staff_id=staff_id)
    db.add(row)
    return row
...
class LateSignInIn(BaseModel):
    client_id:str=Field(min_length=10,max_length=80)
    child_id:str
    room_id:str
    staff_id:str
    effective_at:datetime|None=None
class AlertResolveIn(BaseModel): staff_id:str
class EmergencyPrintSettingsIn(BaseModel):
...
class CentreOperationalSettingsIn(BaseModel):
    parent_history_days:int=Field(ge=1,le=365)
    sleep_check_minutes:int=Field(ge=5,le=10)
...
class ChildAdminUpdateIn(BaseModel):
    ...
    room_id:str|None=None
    active:bool|None=None
    # Never audit, log, or return this confirmation secret.
    account_password:str|None=Field(default=None,min_length=1,max_length=300)
...
def admin_settings(a:Account=Depends(admin_only),db:Session=Depends(get_db)):
    ...
    return {
        ...
        'parent_history_days':centre.parent_history_days,
        'sleep_check_minutes':centre.sleep_check_minutes,
        'attendance_relationship_required':centre.attendance_relationship_required,
        'account_count':db.scalar(select(func.count()).select_from(Account).where(Account.centre_id==a.centre_id))
    }

@app.patch('/api/admin/operational-settings')
def update_operational_settings(body:CentreOperationalSettingsIn,a:Account=Depends(admin_only),db:Session=Depends(get_db)):
    centre=db.get(Centre,a.centre_id)
    before={'parent_history_days':centre.parent_history_days,'sleep_check_minutes':centre.sleep_check_minutes,'attendance_relationship_required':centre.attendance_relationship_required}
    centre.parent_history_days=body.parent_history_days;centre.sleep_check_minutes=body.sleep_check_minutes;centre.attendance_relationship_required=body.attendance_relationship_required
    after={'parent_history_days':centre.parent_history_days,'sleep_check_minutes':centre.sleep_check_minutes,'attendance_relationship_required':centre.attendance_relationship_required}
    audit(db,a.centre_id,'centre',centre.id,'operational_settings_updated',before,after,a.id);db.commit();return after
...
def delete_account(account_id:str,body:AdminDeleteIn,request:Request,a:Account=Depends(admin_only),db:Session=Depends(get_db)):
    ...
    if target.active and target.role=='admin':
        ...
        if not remaining: raise HTTPException(409,'A centre must retain at least one active Admin account')
    references={
        'handled data requests':db.scalar(select(func.count()).select_from(ParentDataRequest).where(ParentDataRequest.handled_by_id==target.id)),
        'audit attribution':db.scalar(select(func.count()).select_from(Audit).where(Audit.centre_id==a.centre_id,Audit.actor_id==target.id)),
    }
...
def bootstrap(a:Account=Depends(operations_account),db:Session=Depends(get_db)):
    ...
    return {
        ...
        'centre':{
            ...
            'timezone':c.timezone,
            'sleep_check_minutes':c.sleep_check_minutes,
            'attendance_relationship_required':c.attendance_relationship_required,
...
def dependency_response(references:dict[str,int],archive_label:str):
    used={name:count for name,count in references.items() if count}
    if used:
        raise HTTPException(409,{
            'code':'has_history',
            'references':used,
            'message':f'Permanent deletion is unavailable because this {archive_label} has history. Archive or deactivate it instead.'
        })

@app.post('/api/admin/children/{child_id}/delete')
def delete_child(child_id:str,body:AdminDeleteIn,a:Account=Depends(admin_only),db:Session=Depends(get_db)):
    verify_admin_password(db,a,body.admin_password)
    child=db.scalar(select(Child).where(Child.id==child_id,Child.centre_id==a.centre_id))
    if not child: raise HTTPException(404,'Child not found')
    display=' '.join(part for part in [child.preferred_name or child.first_name,child.last_name] if part).strip()
    if body.confirm.strip()!=display: raise HTTPException(422,'Type the child name exactly to confirm permanent deletion')
    references={
        'family links':db.scalar(select(func.count()).select_from(ParentChild).where(ParentChild.child_id==child.id)),
        'attendance':db.scalar(select(func.count()).select_from(Attendance).where(Attendance.child_id==child.id)),
        'room visits':db.scalar(select(func.count()).select_from(RoomVisit).where(RoomVisit.child_id==child.id)),
        'care records':db.scalar(select(func.count()).select_from(Event).where(Event.child_id==child.id)),
        'sleep sessions':db.scalar(select(func.count()).select_from(SleepSession).where(SleepSession.child_id==child.id)),
        'sleep checks':db.scalar(select(func.count()).select_from(SleepCheck).where(SleepCheck.child_id==child.id)),
        'medication authorities':db.scalar(select(func.count()).select_from(MedicationAuthority).where(MedicationAuthority.child_id==child.id)),
        'medication administrations':db.scalar(select(func.count()).select_from(MedicationAdministration).where(MedicationAdministration.child_id==child.id)),
        'incidents':db.scalar(select(func.count()).select_from(Incident).where(or_(Incident.child_id==child.id,Incident.other_child_id==child.id))),
        'parent notes':db.scalar(select(func.count()).select_from(ParentNote).where(ParentNote.child_id==child.id)),
        'data requests':db.scalar(select(func.count()).select_from(ParentDataRequest).where(ParentDataRequest.child_id==child.id)),
    }
    dependency_response(references,'child')
    child_id_copy=child.id
    db.delete(child)
    audit(db,a.centre_id,'child',child_id_copy,'deleted',before={'name':display},actor=a.id,reason='Password-confirmed admin deletion')
    db.commit()
    return {'ok':True}
...
@app.post('/api/admin/staff/{staff_id}/delete')
def delete_staff(staff_id:str,body:AdminDeleteIn,a:Account=Depends(admin_only),db:Session=Depends(get_db)):
    verify_admin_password(db,a,body.admin_password)
    staff=db.scalar(select(Staff).where(Staff.id==staff_id,Staff.centre_id==a.centre_id))
    if not staff: raise HTTPException(404,'Staff member not found')
    display=' '.join(part for part in [staff.preferred_name or staff.first_name,staff.last_name] if part).strip()
    if body.confirm.strip()!=display: raise HTTPException(422,'Type the teacher name exactly to confirm permanent deletion')
    references={
        'attendance':db.scalar(select(func.count()).select_from(Attendance).where(Attendance.recorded_by_staff_id==staff.id)),
        'room visits':db.scalar(select(func.count()).select_from(RoomVisit).where(or_(RoomVisit.started_by_staff_id==staff.id,RoomVisit.ended_by_staff_id==staff.id))),
        'care records':db.scalar(select(func.count()).select_from(Event).where(or_(Event.performed_by_id==staff.id,Event.recorded_by_id==staff.id))),
        'sleep sessions':db.scalar(select(func.count()).select_from(SleepSession).where(or_(SleepSession.opened_by_staff_id==staff.id,SleepSession.closed_by_staff_id==staff.id))),
        'sleep checks':db.scalar(select(func.count()).select_from(SleepCheck).where(SleepCheck.staff_id==staff.id)),
        'medication receipts':db.scalar(select(func.count()).select_from(MedicationReceipt).where(or_(MedicationReceipt.received_by_id==staff.id,MedicationReceipt.returned_by_id==staff.id))),
        'medication administrations':db.scalar(select(func.count()).select_from(MedicationAdministration).where(MedicationAdministration.staff_id==staff.id)),
        'incidents':db.scalar(select(func.count()).select_from(Incident).where(or_(Incident.created_by_id==staff.id,Incident.finalised_by_id==staff.id))),
        'incident actions':db.scalar(select(func.count()).select_from(IncidentAction).where(IncidentAction.staff_id==staff.id)),
        'audit attribution':db.scalar(select(func.count()).select_from(Audit).where(Audit.centre_id==a.centre_id,Audit.actor_id==staff.id)),
    }
    dependency_response(references,'teacher')
    copied_id=staff.id
    db.delete(staff)
    audit(db,a.centre_id,'staff',copied_id,'deleted',before={'name':display},actor=a.id,reason='Password-confirmed admin deletion')
    db.commit()
    return {'ok':True}
...
def update_child(
    ...
    if updates:
        verify_account_password(
            db,a,account_password or '',
            'child_update_confirm',
            'Your current account password is required to change a child'
        )
    ...
    if 'middle_name' in updates:
        child.middle_name=updates['middle_name'].strip() if updates['middle_name'] else None
...
@app.get('/api/admin/families/{parent_id}/parent-link')
def parent_login_link(parent_id:str,a:Account=Depends(admin_only),db:Session=Depends(get_db)):
    family=db.scalar(select(Parent).where(Parent.id==parent_id,Parent.centre_id==a.centre_id))
    if not family: raise HTTPException(404,'Family not found')
    origin=os.getenv('PUBLIC_ORIGIN','http://localhost:5173').rstrip('/')
    url=f'{origin}/parent?login={quote(family.login)}'
    try:
        import qrcode
    ...
    return {'url':url,'qr_data_url':qr,'family_login':family.login}
@app.get('/api/admin/parent-login-link')
def generic_parent_login_link(a:Account=Depends(admin_only),db:Session=Depends(get_db)):
    origin=os.getenv('PUBLIC_ORIGIN','http://localhost:5173').rstrip('/');url=f'{origin}/parent'
    try:
        import qrcode
        image=qrcode.make(url);buffer=io.BytesIO();image.save(buffer,format='PNG');qr='data:image/png;base64,'+base64.b64encode(buffer.getvalue()).decode()
    except ImportError:qr=None
    return {'url':url,'qr_data_url':qr}

@app.post('/api/admin/families/{parent_id}/delete')
def delete_family(parent_id:str,body:AdminDeleteIn,a:Account=Depends(admin_only),db:Session=Depends(get_db)):
    verify_admin_password(db,a,body.admin_password)
    parent_row=db.scalar(select(Parent).where(Parent.id==parent_id,Parent.centre_id==a.centre_id))
    if not parent_row: raise HTTPException(404,'Family not found')
    if body.confirm.strip()!=parent_row.name: raise HTTPException(422,'Type the family name exactly to confirm permanent deletion')
    references={
        'parent notes':db.scalar(select(func.count()).select_from(ParentNote).where(ParentNote.centre_id==a.centre_id,ParentNote.child_id.in_(select(ParentChild.child_id).where(ParentChild.parent_id==parent_row.id)))),
        'data requests':db.scalar(select(func.count()).select_from(ParentDataRequest).where(ParentDataRequest.parent_id==parent_row.id)),
        'signatures':db.scalar(select(func.count()).select_from(Signature).where(Signature.parent_id==parent_row.id)),
    }
    dependency_response(references,'family')
    revoked=revoke_parent_sessions(db,parent_row.id)
    db.execute(delete(ParentChild).where(ParentChild.parent_id==parent_row.id))
    parent_id_copy=parent_row.id; name=parent_row.name
    db.delete(parent_row)
    audit(db,a.centre_id,'parent',parent_id_copy,'deleted',before={'name':name,'sessions_revoked':revoked},actor=a.id,reason='Password-confirmed admin deletion')
    db.commit()
    return {'ok':True,'sessions_revoked':revoked}
...
def safety_expected_children(db:Session,centre_id:str,room_id:str):
    ...
    for attendance in attendances:
        physical=attendance.visit_room_id if attendance.visit_room_id and attendance.visit_ended_at is None else attendance.room_id
        if physical==room_id and attendance.child_id not in ids:ids.append(attendance.child_id)
    children=[db.get(Child,child_id) for child_id in ids]
    return [{'id':child.id,'name':' '.join(part for part in [child.preferred_name or child.first_name,child.last_name] if part)} for child in children if child and child.centre_id==centre_id]

def safety_check_out(check:CentreSafetyCheck,db:Session,include_live:bool=True):
    staff=db.get(Staff,check.staff_id);records=list(db.scalars(select(CentreSafetyCheckRoom).where(CentreSafetyCheckRoom.safety_check_id==check.id).order_by(CentreSafetyCheckRoom.checked_at)))
    saved={record.room_id:record for record in records};rooms=list(db.scalars(select(Room).where(Room.centre_id==check.centre_id).order_by(Room.name)))
    room_rows=[]
    for room in rooms:
        record=saved.get(room.id)
...
def safety_check_history(a:Account=Depends(operations_account),db:Session=Depends(get_db)):
    checks=list(db.scalars(select(CentreSafetyCheck).where(CentreSafetyCheck.centre_id==a.centre_id).order_by(CentreSafetyCheck.started_at.desc()).limit(20)))
    return [safety_check_out(check,db,include_live=check.status=='open') for check in checks]

@app.post('/api/admin/safety-checks')
def start_safety_check(body:SafetyCheckStartIn,a:Account=Depends(operations_account),db:Session=Depends(get_db)):
    staff=db.scalar(select(Staff).where(Staff.id==body.staff_id,Staff.centre_id==a.centre_id,Staff.active.is_(True)))
    if not staff:raise HTTPException(404,'Choose an active checker')
    verify_staff_pin(db,staff,body.staff_pin,'Checker PIN is incorrect')
    check=CentreSafetyCheck(centre_id=a.centre_id,staff_id=staff.id,account_id=a.id,status='open');db.add(check);db.flush()
    audit(db,a.centre_id,'centre_safety_check',check.id,'started',after={'staff_id':staff.id,'account_id':a.id},actor=a.id);db.commit()
    return safety_check_out(check,db)

@app.get('/api/admin/safety-checks/{check_id}')
def get_safety_check(check_id:str,a:Account=Depends(operations_account),db:Session=Depends(get_db)):
    check=db.scalar(select(CentreSafetyCheck).where(CentreSafetyCheck.id==check_id,CentreSafetyCheck.centre_id==a.centre_id))
    if not check:raise HTTPException(404,'Safety check not found')
    return safety_check_out(check,db,include_live=check.status=='open')

@app.post('/api/admin/safety-checks/{check_id}/rooms/{room_id}')
def confirm_safety_check_room(check_id:str,room_id:str,body:SafetyCheckRoomIn,a:Account=Depends(operations_account),db:Session=Depends(get_db)):
    check=db.scalar(select(CentreSafetyCheck).where(CentreSafetyCheck.id==check_id,CentreSafetyCheck.centre_id==a.centre_id))
    if not check:raise HTTPException(404,'Safety check not found')
    if check.status!='open':raise HTTPException(409,'This safety check is complete')
    room=db.scalar(select(Room).where(Room.id==room_id,Room.centre_id==a.centre_id))
    if not room:raise HTTPException(404,'Room not found')
    if db.scalar(select(CentreSafetyCheckRoom).where(CentreSafetyCheckRoom.safety_check_id==check.id,CentreSafetyCheckRoom.room_id==room.id)):raise HTTPException(409,'This Room count is already confirmed')
    expected=safety_expected_children(db,a.centre_id,room.id)
    if body.expected_count!=len(expected):raise HTTPException(409,'The system count changed while this Room was being checked. Please recount.')
    if now()-utc(check.started_at)>timedelta(minutes=30):
        staff=db.scalar(select(Staff).where(Staff.id==check.staff_id,Staff.centre_id==a.centre_id,Staff.active.is_(True)))
        if not staff:raise HTTPException(409,'The original checker is no longer active. Start a new safety check.')
        verify_staff_pin(db,staff,body.staff_pin,'Re-enter the checker PIN to continue this older safety check')
    if body.observed_count!=len(expected) and len((body.note or '').strip())<3:raise HTTPException(422,'Add a short note before confirming a mismatch')
    record=CentreSafetyCheckRoom(safety_check_id=check.id,centre_id=a.centre_id,room_id=room.id,room_name=room.name,expected_count=len(expected),observed_count=body.observed_count,expected_children=expected,staff_id=check.staff_id,note=(body.note or '').strip() or None)
    db.add(record);db.flush();audit(db,a.centre_id,'centre_safety_check_room',record.id,'confirmed',after={'safety_check_id':check.id,'room_id':room.id,'expected_count':record.expected_count,'observed_count':record.observed_count,'match':record.expected_count==record.observed_count,'staff_id':check.staff_id},actor=a.id);db.commit()
    return safety_check_out(check,db)

@app.post('/api/admin/safety-checks/{check_id}/complete')
def complete_safety_check(check_id:str,a:Account=Depends(operations_account),db:Session=Depends(get_db)):
    check=db.scalar(select(CentreSafetyCheck).where(CentreSafetyCheck.id==check_id,CentreSafetyCheck.centre_id==a.centre_id))
    if not check:raise HTTPException(404,'Safety check not found')
    if check.status!='open':return safety_check_out(check,db,include_live=False)
    room_count=db.scalar(select(func.count()).select_from(Room).where(Room.centre_id==a.centre_id));checked=db.scalar(select(func.count()).select_from(CentreSafetyCheckRoom).where(CentreSafetyCheckRoom.safety_check_id==check.id))
    if checked!=room_count:raise HTTPException(409,'Confirm every Room before completing the safety check')
    check.status='completed';check.completed_at=now();audit(db,a.centre_id,'centre_safety_check',check.id,'completed',after={'staff_id':check.staff_id,'rooms_checked':checked},actor=a.id);db.commit()
    return safety_check_out(check,db,include_live=False)
...
def activity_names(db:Session,child_id:str|None=None,room_id:str|None=None,staff_id:str|None=None,centre_id:str|None=None):
    child=db.scalar(select(Child).where(Child.id==child_id,Child.centre_id==centre_id)) if child_id and centre_id else (db.get(Child,child_id) if child_id else None);room=db.scalar(select(Room).where(Room.id==room_id,Room.centre_id==centre_id)) if room_id and centre_id else (db.get(Room,room_id) if room_id else None);staff=db.scalar(select(Staff).where(Staff.id==staff_id,Staff.centre_id==centre_id)) if staff_id and centre_id else (db.get(Staff,staff_id) if staff_id else None)
    return {'child_name':' '.join(part for part in [child.preferred_name or child.first_name,child.last_name] if part) if child else None,'room':room.name if room else None,'teacher':(staff.preferred_name or staff.first_name) if staff else None}

def activity_device(db:Session,centre_id:str,device_id:str|None):
    item=db.scalar(select(Device).where(Device.id==device_id,Device.centre_id==centre_id)) if device_id else None
    return {'device_id':item.id if item else device_id,'device':item.label if item else None,'device_mode':item.mode if item else None}

AUDIT_STAFF_ID_FIELDS={'staff_id','performed_by_id','recorded_by_staff_id','started_by_staff_id','ended_by_staff_id','opened_by_staff_id','closed_by_staff_id'}
AUDIT_ACCOUNT_ID_FIELDS={'actor_id','account_id','viewer_account_id'}
AUDIT_TECHNICAL_ONLY_FIELDS={'safety_check_id','attendance_id','note_id'}
def audit_display_data(db:Session,centre_id:str,value:dict):
    display={key:item for key,item in value.items() if key not in AUDIT_TECHNICAL_ONLY_FIELDS}
    for key,identifier in value.items():
        if key in AUDIT_TECHNICAL_ONLY_FIELDS:continue
        if not identifier:continue
        if key=='room_id':
            row=db.scalar(select(Room).where(Room.id==identifier,Room.centre_id==centre_id));display[key]=row.name if row else 'Unavailable'
        elif key=='child_id':
            row=db.scalar(select(Child).where(Child.id==identifier,Child.centre_id==centre_id));display[key]=' '.join(part for part in [row.preferred_name or row.first_name,row.last_name] if part) if row else 'Unavailable'
        elif key in AUDIT_STAFF_ID_FIELDS:
            row=db.scalar(select(Staff).where(Staff.id==identifier,Staff.centre_id==centre_id));display[key]=' '.join(part for part in [row.preferred_name or row.first_name,row.last_name] if part) if row else 'Unavailable'
        elif key=='device_id':
            row=db.scalar(select(Device).where(Device.id==identifier,Device.centre_id==centre_id));display[key]=row.label if row else 'Unavailable'
        elif key in AUDIT_ACCOUNT_ID_FIELDS:
            row=db.scalar(select(Account).where(Account.id==identifier,Account.centre_id==centre_id));display[key]=row.login_id if row else 'Unavailable'
    return display

def audit_activity_out(item:Audit,db:Session):
    after=clean_domain_data(item.after) or {};before=clean_domain_data(item.before) or {};values={**before,**after}
    room_id=values.get('room_id');child_id=values.get('child_id');staff_id=values.get('staff_id') or values.get('performed_by_id');device_id=values.get('device_id')
    if item.entity=='room' and not room_id:room_id=item.entity_id
    if item.entity=='child' and not child_id:child_id=item.entity_id
    if item.entity=='device' and not device_id:device_id=item.entity_id
    actor=db.scalar(select(Account).where(Account.id==item.actor_id,Account.centre_id==item.centre_id)) if item.actor_id else None
    names=activity_names(db,child_id,room_id,staff_id,item.centre_id);device_values=activity_device(db,item.centre_id,device_id)
    return {'id':item.id,'source':'audit','category':audit_category(item),'type':item.action,'activity':f"{item.entity.replace('_',' ')} {item.action.replace('_',' ')}",'entity':item.entity,'entity_id':item.entity_id,'effective_at':item.created_at,'recorded_at':item.created_at,'data':after,'before':before,'display_data':audit_display_data(db,item.centre_id,after),'display_before':audit_display_data(db,item.centre_id,before),'reason':item.reason,'actor_id':item.actor_id,'actor':actor.login_id if actor else ('System' if not item.actor_id else None),'actor_kind':'account' if actor else 'system','corrected':False,**names,**device_values}
...
def activity_classroom_rows(db:Session,centre_id:str,limit:int,child_id:str|None=None,room_id:str|None=None,staff_id:str|None=None,type:str|None=None,start:datetime|None=None,end:datetime|None=None):
    ...
    note_q=select(ParentNote).where(ParentNote.centre_id==centre_id)
    if type and type!='parent_note':note_q=note_q.where(False)
    if child_id:note_q=note_q.where(ParentNote.child_id==child_id)
    if room_id or staff_id:note_q=note_q.where(False)
    if start:note_q=note_q.where(ParentNote.created_at>=start)
    if end:note_q=note_q.where(ParentNote.created_at<end)
    for note in db.scalars(note_q.order_by(ParentNote.created_at.desc()).limit(limit)):
        rows.append({'id':note.id,'source':'parent_note','category':'classroom','type':'parent_note','activity':'parent note','child_id':note.child_id,'room_id':None,'staff_id':None,'effective_at':note.created_at,'recorded_at':note.created_at,'data':{'body':soften_parent_text(note.body)},'original_available':True,'corrected':False,'finalised':True,**activity_names(db,note.child_id)})
    return rows
...
def activity(from_date:date|None=None,to_date:date|None=None,room_id:str|None=None,staff_id:str|None=None,child_id:str|None=None,type:str|None=None,category:Literal['classroom','management','security']|None=None,search:str|None=None,limit:int=100,a:Account=Depends(operations_account),db:Session=Depends(get_db)):
    ...
    if a.role=='admin' and category in {'management','security'}:
        ...
        q=q.where(~and_(Audit.entity=='parent_note',Audit.action=='submitted'))
        audits=list(db.scalars(q.order_by(Audit.created_at.desc()).limit(candidate_limit)))
        rows=[audit_activity_out(item,db) for item in audits]
    if search:
        needle=search.strip().lower();rows=[row for row in rows if needle in ' '.join(str(value) for value in [row.get('type'),row.get('activity'),row.get('child_name'),row.get('room'),row.get('teacher'),row.get('entity'),row.get('data'),row.get('display_data'),row.get('reason'),row.get('actor'),row.get('device')]).lower()]
...
def admin_attendance_signature(attendance_id:str,purpose:Literal['kiosk_sign_in','parent_missing_sign_in_confirmation','kiosk_sign_out','parent_missing_sign_out_confirmation']='kiosk_sign_in',a:Account=Depends(operations_account),db:Session=Depends(get_db)):
    ...
    return {'id':signature.id,'signature_data':signature.signature_data,'relationship':soften_parent_text(signature.relationship),'signed_at':signature.signed_at,'purpose':signature.purpose}

@app.get('/api/admin/parent-notes/{note_id}/original')
def admin_parent_note_original(note_id:str,a:Account=Depends(admin_only),db:Session=Depends(get_db)):
    note=db.scalar(select(ParentNote).where(ParentNote.id==note_id,ParentNote.centre_id==a.centre_id))
    if not note:raise HTTPException(404,'Parent note not found')
    audit(db,a.centre_id,'parent_note',note.id,'parent_note_original_viewed',after={'note_id':note.id,'child_id':note.child_id,'viewer_account_id':a.id},actor=a.id)
    db.commit();return {'id':note.id,'body':note.body,'viewed_at':now()}
...
def classroom_parent_notes(d:Device=Depends(classroom_device),db:Session=Depends(get_db)):
    ...
    return [{'id':n.id,'child_id':n.child_id,'child_name':(db.get(Child,n.child_id).preferred_name or db.get(Child,n.child_id).first_name),'body':soften_parent_text(n.body),'created_at':n.created_at,'read_at':n.read_at,'pinned':n.pinned} for n in rows]
...
def late_sign_in(body:LateSignInIn,d:Device=Depends(classroom_device),db:Session=Depends(get_db)):
    ...
    if not child: raise HTTPException(404,'Child not found')
    prior=db.scalar(select(DomainOperation).where(DomainOperation.centre_id==d.centre_id,DomainOperation.domain=='late_sign_in',DomainOperation.client_operation_id==body.client_id))
...
def attendance_kiosk(body:AttendanceKioskIn,d:Device=Depends(attendance_device),db:Session=Depends(get_db)):
    ...
    if body.action=='sign_in':
        ...
        active=Attendance(centre_id=d.centre_id,child_id=child.id,room_id=room_id,arrived_at=when,device_id=d.id,source='parent_kiosk',signer_name=body.signer_name.strip() if body.signer_name else None,signer_relationship=relationship)
...
def create_event(body:EventIn,d:Device=Depends(classroom_device),db:Session=Depends(get_db)):
    ...
    db.flush()
    if body.type in {'nappy','toilet'}:
        requested={'nappies_low':'nappies_low','nappies_out':'nappies_out','spare_clothes_needed':'spare_clothes_needed','soiled_clothes':'soiled_clothes'}
        for event in items:
            for field,alert_type in requested.items():
                if data.get(field):touch_alert(db,d.centre_id,event.child_id,alert_type,event,staff.id)
...
def sleep(body:SleepIn,d:Device=Depends(classroom_device),db:Session=Depends(get_db)):
    ...
    for child in children:
        ...
        if body.action=='put_down':
            ...
            elif body.action=='wake_and_got_up':
                # One idempotent DomainOperation closes a currently sleeping
                # session; never emulate this as two unrelated client calls.
                if not session.fell_asleep_at or session.woke_at: raise HTTPException(409,f'{child.first_name} must currently be sleeping')
                if utc(when)<utc(session.fell_asleep_at):raise HTTPException(409,'Wake time cannot be before fell asleep time')
                session.woke_at=when;session.got_up_at=when;session.closed_by_staff_id=staff.id
                session.wake_state=body.wake_state;session.quality=body.quality;session.note=body.note or session.note
...
def parent_day(child_id:str,day:date|None=None,p:Parent=Depends(parent),db:Session=Depends(get_db)):
    ...
    for a in attendance:
        ...
        attendance_out.append({
            ...
            'device':attendance_device.label if attendance_device else None,
    ...
    return {
        ...
        'alerts':[alert_out(row,db) for row in db.scalars(select(ChildAlert).where(ChildAlert.centre_id==p.centre_id,ChildAlert.child_id==child_id,ChildAlert.resolved_at.is_(None)).order_by(ChildAlert.created_at.desc()))]
    }

@app.post('/api/parent/notes')
def parent_note(body:NoteIn,p:Parent=Depends(parent),db:Session=Depends(get_db)):
    if not db.scalar(select(ParentChild).where(ParentChild.parent_id==p.id,ParentChild.child_id==body.child_id)):raise HTTPException(404,'Child not found')
    n=ParentNote(centre_id=p.centre_id,child_id=body.child_id,body=body.body);db.add(n);db.flush();audit(db,p.centre_id,'parent_note',n.id,'submitted',after={'note_id':n.id,'child_id':n.child_id},actor=p.id);db.commit();return {'id':n.id,'created_at':n.created_at}
...
def export(child_id:str,day:date|None=None,p:Parent=Depends(parent),db:Session=Depends(get_db)):
    ...
    import csv,io
    record=parent_day(child_id,day,p,db);centre=db.get(Centre,p.centre_id);zone=ZoneInfo(centre.timezone);child=db.get(Child,child_id)
    def local_time(value):
        if not value:return ''
        parsed=value if isinstance(value,datetime) else datetime.fromisoformat(str(value).replace('Z','+00:00'))
        return utc(parsed).astimezone(zone).isoformat(timespec='seconds')
    def safe_slug(value):
        value=re.sub(r'[^a-z0-9]+','-',value.lower()).strip('-')
        return value or 'child'
    record_date=(day or date.fromisoformat(record['date'])).isoformat()
    exported_at=now().astimezone(zone).strftime('%Y-%m-%d-%H%M')
    child_slug=safe_slug(' '.join(part for part in [child.preferred_name or child.first_name,child.last_name] if part))
    filename=f'{child_slug}_daily-care_{record_date}_exported-{exported_at}.csv'
    record=parent_day(child_id,day,p,db);out=io.StringIO();w=csv.writer(out);w.writerow(['effective_time','type','summary','room','performed_by','source','relationship','signature_recorded'])
