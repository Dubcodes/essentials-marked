import React,{useEffect,useState}from'react';import{createRoot}from'react-dom/client';import{api}from'./api';import Classroom from'./classroom/Classroom';import PairTablet from'./classroom/Pair';import ParentView from'./parent/Parent';import AdminConsole from'./admin/AdminConsole';import TeacherLanding from'./account/TeacherLanding';import{Login}from'./account/Login';import{accountLandingForRole,quickNavForRole}from'./account/role-ui';import{ErrorBoundary}from'./ErrorBoundary';import'./style.css';import'./classroom.css';import'./admin.css';import'./emergency.css';import'./branding.css';
...
function AdminGate(){const[state,setState]=useState<'loading'|'ready'|'login'|'teacher'>('loading'),[account,setAccount]=useState<any>();useEffect(()=>{void api('/auth/account/me').then((value:any)=>{setAccount(value);setState(accountLandingForRole(value.role)==='teacher'?'teacher':'ready')}).catch(()=>setState('login'))},[]);if(state==='loading')return <main className="loading">Opening account…</main>;if(state==='login')return <Login parent={false} onDone={()=>location.reload()}/>;if(state==='teacher')return <TeacherLanding account={account}/>;return <AdminConsole/>}
...
function QuickNav(){
  ...
  useEffect(()=>{
    ...
    void api('/auth/account/me').then(setAccount).catch(()=>setAccount(null));
  },[parentContext]);

  const links=quickNavForRole(account?.role,parentContext);
  ...
  return(
    ...
    >
      ...
      {account?.role==='teacher'&&
        <button
          className="minor"
          onClick={()=>void api('/auth/account/logout',{method:'POST'}).finally(()=>location.assign('/'))}
        >
          Sign out
        </button>
      }
