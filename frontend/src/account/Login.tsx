...
export function Login({parent,onDone}:{parent:boolean;onDone:()=>void}){
  const[id,setId]=useState(()=>parent?new URLSearchParams(location.search).get('login')||'':''),[secret,setSecret]=useState(''),[error,setError]=useState(''),[saving,setSaving]=useState(false);
